...
describe Cassy::ServiceTicket do
  ...
  it "should validate" do
    expect(Cassy::ServiceTicket.validate("http://members.something.com","ST-12345678901234567890")).to eq([@service_ticket, "Ticket 'ST-12345678901234567890' for 'http://members.something.com' for user '1' successfully validted."]    )
  end

  it "should not validate if the ticket has already been consumed" do
    @service_ticket.consume!
    expect(Cassy::ServiceTicket.validate("http://members.something.com","ST-12345678901234567890")).to eq([nil, "Ticket ST-12345678901234567890 has already been consumed."])
  end

  it "should not validate if the ticket is too old" do
    @service_ticket.created_on = Time.now-7201
    @service_ticket.save!
    expect(Cassy::ServiceTicket.validate("http://members.something.com", "ST-12345678901234567890")).to eq([nil, "Ticket ST-12345678901234567890 has expired. Please try again."])
  end

  it "should not validate if the ticket is invalid" do
    expect(Cassy::ServiceTicket.validate("http://members.something.com", "ST-09876543210987654321")).to eq([nil, "Ticket 'ST-09876543210987654321' not recognized."])
  end
  ...
  describe "#send_logout_notification" do
    it "should send valid single sign out XML to the service URL" do
      ...
      expect(a_request(:post, 'example.com').with{ |req|
